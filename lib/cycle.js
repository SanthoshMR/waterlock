    ...
    loginSuccess: function(req, res, user){
      ...
      if (postResponse === 'jwt') {
        ...
        Jwt.create({token: jwtData.token, uses: 0, owner: user.id}).exec(function(err){
          ...
          delete req.session.user.auth.password;

          User.findOne(user.id).populate("roles").exec(function(err, userWithRoles) {
            if(err) {
              return res.serverError('Couldn\'t find roles associated with the user');
            }
